...
class PlainNet(nn.Module):
    ...
    def __init__(self, block_list, pool, num_classes, **kwargs):
        """
        Args:
            block_list (List[nn.Module]): list of
                blocks for feature extraction.
            pool (nn.Module): pooling layer,
                will be used after feature extraction.
        ...
        self.last_channels = self.pool.out_channels
        self.fc_linear = nn.Linear(self.last_channels, self.num_classes, bias=True)

    def extract_features(self, x):
        """Extract convolition features.

        Args:
            x (torch.Tensor): input data

        Returns:
            torch.Tensor with convolution features.
        """
        output = x
    ...
    def from_string(num_classes, plainnet_struct):
        """Generate model from string.

        Args:
            num_classes (int): number of classes to predict.
            plainnet_struct (str): model configuration.

        Returns:
            model (nn.Module) from configuration.
        """
...
def genet_large(
    ...
    Args:
        pretrained (bool): if true then will load checkpoint from
            ``<root>/GENet_large.pth``.
            Default is ``False``.
