...
class BasicBlock(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, head_conv=1, norm_layer=None
    ):
        ...
        self.conv1 = nn.Conv3d(
            inplanes, planes, kernel_size=3, stride=stride, bias=False
        )
...
class SlowFast(nn.Module):
    def __init__(
        self, block=Bottleneck, layers=[3, 4, 6, 3], class_num=10, dropout=0.5
    ):
        ...
        self.fast_maxpool = nn.MaxPool3d(
        ...
        self.slow_maxpool = nn.MaxPool3d(
            kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1)
        )
    ...
    def _make_layer_fast(self, block, planes, blocks, stride=1, head_conv=1):
        ...
        layers = []
        layers.append(
            block(self.fast_inplanes, planes, stride, downsample, head_conv=head_conv)
        )
    ...
    def _make_layer_slow(self, block, planes, blocks, stride=1, head_conv=1):
        ...
        layers = []
        layers.append(
            block(self.slow_inplanes, planes, stride, downsample, head_conv=head_conv)
        )
