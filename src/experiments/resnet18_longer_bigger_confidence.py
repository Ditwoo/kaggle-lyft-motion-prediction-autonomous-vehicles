...
from src.batteries import make_checkpoint, save_checkpoint
...
def get_loaders(train_batch_size=32, valid_batch_size=64):
    ...
    # n_samples = train_batch_size * 100
...
def train_fn(
    ...
    model.train()
    metrics = {"loss": 0.0}
    save_batches = [int(len(loader) * pcnt) for pcnt in np.arange(0.1, 1.0, 0.1)]
        ...
        for idx, batch in enumerate(loader):
            ...
            if (idx + 1) in save_batches and logdir is not None:
                checkpoint = make_checkpoint(
                    stage="_train", epoch=(idx + 1), model=model, optimizer=optimizer
                )
                save_checkpoint(checkpoint, logdir / "_train", f"batch_{idx+1}.pth")
    ...
    with TensorboardLogger(tb_dir) as tb:
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            train_metrics = train_fn(
                model, train_loader, device, criterion, optimizer, logdir=logdir
            )
...
def main() -> None:
    experiment_name = "resnet18_longer_bigger_confidence"
