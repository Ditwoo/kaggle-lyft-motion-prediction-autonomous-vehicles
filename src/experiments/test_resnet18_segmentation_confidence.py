...
from src.models.resnets import resnet18_unet
            ...
            if tensorboard_logger is not None:
                ...
                if (idx + 1) % 1_000 == 0:
                    ...
                    tensorboard_logger.writer.add_images(
                        "gt_vs_mask",
                        torch.cat([masks, torch.sigmoid(masks_logits)], dim=-1),
                        idx,
                    )
...
def valid_fn(model, loader, device, ground_truth_file, logdir, verbose=True):
    ...
    with torch.no_grad(), tqdm(
        ...
        for idx, batch in enumerate(loader):
            ...
            predictions, confidences, _ = model(images)
...
def log_metrics(
    ...
    """
    order = ("regression_loss", "mask_loss", "loss")
...
def main() -> None:
    experiment_name = "test_resnet18_segmentation_confidence"
