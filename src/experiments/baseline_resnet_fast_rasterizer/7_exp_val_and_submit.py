...
exp_basename = "resnet34_frast_fulldata_confidence_25hist_accel_cnt"
...
test_dataset = AccelAgentDataset(cfg, test_zarr, rasterizer, agents_mask=test_mask)
...
with tqdm(total=len(test_dataloader)) as progress:
    for batch in test_dataloader:
        ...
        preds, confidences = model(
            batch["image"].to(device), batch["xy_acceleration"].to(device)
        )
